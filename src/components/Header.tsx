...
const Header = () => {
  ...
  // Реф для получения ссылки на элемент заголовка
  const headerRef = useRef<HTMLElement>(null);
  // Состояние для хранения поискового запроса
  const [searchTerm, setSearchTerm] = useState("");
  // Состояние для отображения/скрытия поля ввода поиска
  const [showInput, setShowInput] = useState(false);
  // Хук для навигации
  const navigate = useNavigate();
  // Хук для получения текущего пути URL
  const location = useLocation();
  // Реф для получения ссылки на инпут поиска
  const searchInputRef = useRef<HTMLInputElement>(null);

  // Обработчик клика по бургер-меню (отображение/скрытие)
  const handleBurgerClick = () => {
    setIsBurgerVisible((prevIsToggled) => !prevIsToggled);
  };

  // Обработчик изменения значения в поле ввода поиска
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setSearchTerm(event.target.value);
  };

  // Обработчик клика по кнопке поиска (отображение/скрытие поля ввода)
  ...
  useEffect(() => { // Использование хука эффекта для фокусировки на поле ввода при его отображении
    if (showInput && searchInputRef.current) { // Если поле ввода отображается и ссылка на него существует...
      searchInputRef.current.focus(); // ...устанавливается фокус на поле ввода
    }
  }, [showInput]); // Эффект зависит от состояния showInput

  // Обработчик отправки формы поиска
  const handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); // Предотвращаем перезагрузку страницы
    navigate(`/search?q=${searchTerm}`); // Переходим на страницу поиска с заданным запросом
    setShowInput(prevIsToggled => !prevIsToggled); // Скрываем поле ввода
    setSearchTerm(""); // Очищаем поле ввода после перехода
  };

  // Хук useEffect для обработки клика вне элемента заголовка
  useEffect(() => {
    // Функция, которая будет вызываться при клике на документе
    function handleClickOutside(event: MouseEvent) {
      // Если клик произошел вне элемента заголовка, скрываем бургер-меню
      if (
        headerRef.current &&
    ...
    // Добавляем обработчик события 'mousedown' на документ
    document.addEventListener("mousedown", handleClickOutside);
    // Возвращаем функцию для удаления обработчика при размонтировании компонента
    return () => {
      document.removeEventListener("mousedown", handleClickOutside);
    };
    // Зависимость от isBurgerVisible, чтобы эффект срабатывал при изменении состояния
  }, [isBurgerVisible]);

  return (
    <header
      className={`header ${
        location.pathname === "/home" ? "header__home" : ""
      }`}
      ref={headerRef}
    >
      <div className="main-container header__container">
        ...
        >
          <svg id="svg8" version="1.1" viewBox="0 45 92 92">
            <path id="path2" d="M0 0h92v92H0z" fill="transparent" />
            <path
        ...
        <div className="search__container">
          ...
          >
            <svg viewBox="0 0 24 24" className="button-search__icon">
              <path
